...
function cleanSku(v = "") {
  return v.toLocaleLowerCase().replace(/[^0-9a-zA-Z]/gi, "");
}

function advanceMatch(sku, styleId) {
  const nSku = cleanSku(sku);
  const nStyleId = cleanSku(styleId);
  return nSku === nStyleId;
}
...
function isSku(sku = "", styleId = "") {
  ...
  if (normalMatch(sku, styleId)) {
    return true;
  }

  return advanceMatch(sku, styleId);
}
